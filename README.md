...
## Prueba local de la aplicación sin dockerizar
...
Modificar la aplicación para añadir una nueva ruta (`status`) que muestre la versión de la aplicación junto con su nombre y apellidos.
...
@app.get("/status")
def version():
    return {"status": "Apellidos, Nombre - v.x.y"}
```

## Dockerizar

1. Construir la imagen dockerizada de la aplicación
2. Probar la imagen usando el fichero de despligue proporcionado.

## Pruebas: ejemplo de uso con `curl`
